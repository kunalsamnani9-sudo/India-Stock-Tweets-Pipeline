# pw_collect_india_stocks_with_state_no_blocking.py
"""
Playwright scraper — simplified version with CAPTCHA/block detection removed.

Usage:
  python pw_collect_india_stocks_with_state_no_blocking.py
...
def run():
    ...
    with sync_playwright() as p:
        ...
        while total_written < GLOBAL_MAX_TWEETS and any(scrolls_done[tag] < MAX_SCROLLS_PER_HASHTAG for tag in KEYWORDS):
            ...
            log.info("Visiting %s (round-robin). Total collected so far: %d", tag, total_written)
            collected = 0
            scrolls = 0
            q = tag
            ...
            while scrolls < MAX_SCROLLS_PER_HASHTAG and total_written < GLOBAL_MAX_TWEETS:
                ...
                scrolls += 1
                if scrolls % 50 == 0:
                    log.info("Scrolled %d times for %s, collected %d/%d", scrolls, tag, collected, VISIT_SCROLLS)

            log.info("Finished %s: collected %d (scrolls=%d)", tag, collected, scrolls)
    ...
    log.info("TOTAL collected: %d (requested %d)", total, sum([GLOBAL_MAX_TWEETS for _ in KEYWORDS]))
    print("\n[+] Saved file:", OUT_CSV)
